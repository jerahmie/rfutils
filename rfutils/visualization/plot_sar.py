...
def plot_sar(plot_filename, xdim, ydim, zdim, sar, xcenter=(0,0,0), mask=None, dims=None):
    ...
    ax[0].pcolormesh(np.transpose(XX), np.transpose(YY), sar[xind1:xind2,yind1:yind2,zind0], cmap=sar_cmap, vmin=0.00001)
    ax[0].set_aspect('equal','box')
    XX,ZZ = np.meshgrid(xdim[xind1:xind2], zdim[zind1:zind2])
    ax[1].pcolormesh(np.transpose(XX), np.transpose(ZZ), np.rot90(sar[xind1:xind2,yind0,zind1:zind2],2), cmap=sar_cmap, vmin=0.00001)
    ax[1].set_aspect('equal','box')
    YY,ZZ = np.meshgrid(ydim[yind1:yind2], zdim[zind1:zind2])
    im = ax[2].pcolormesh(ZZ,YY, np.fliplr(np.rot90(sar[xind0,yind1:yind2,zind1:zind2],3)), cmap=sar_cmap, vmin=0.00001)
...
if __name__ == "__main__":
    if sys.platform == 'win32':
        base_dir = os.path.join(r'D:', os.sep, r'Temp_CST',
                                    ...
                                    r'Export',  r'3d', r'Vopgen')
        sar_file_dir = os.path.join(base_dir, r'Output')
    
    sarmask_dict = hdf5storage.loadmat(os.path.join(base_dir, 'sarmask_aligned.mat'))
