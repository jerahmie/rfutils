...
def apply_shim(field, magnitudes, phases, rot_dir=0):
    ...
    for ch in range(field_shape[-1]):
        fields_shimmed += np.multiply(fields_component[:,:,:,ch], np.exp(1.0j*phases[ch]), dtype=complex)
...
def plot_fields_shimmed(field3d, xdim, ydim, zdim, slice_xyz, mask=None, 
    ...
    axs[0].pcolormesh(XX, YY, field3d[:,:,z_ind], vmin=field_min, vmax=field_max)
    axs[0].autoscale(enable=True, axis='both', tight=True)
    axs[0].set_aspect('equal','box')
    axs[0].set_title('Axial')
    XX, YY = np.meshgrid(xdim, zdim)
    axs[1].pcolormesh(XX,YY,field3d[:,y_ind,:], vmin=field_min, vmax=field_max)
    axs[1].autoscale(enable=True, axis='both', tight=True)
    axs[1].set_aspect('equal','box')
    axs[1].set_title('Sagittal')
    XX, YY = np.meshgrid(ydim, zdim)
    im = axs[2].pcolormesh(XX, YY, field3d[x_ind,:,:], vmin=field_min, vmax=field_max)
...
if __name__ == "__main__":
    ...
    cp_phases_top = [-2.0*np.pi/nchannels * ch  for ch in range(0,nchannels,2)]
    cp_phases_bottom = [-2.0*np.pi/nchannels * ch for ch in range(1,nchannels,2)]
    cp_phases = cp_phases_top + cp_phases_bottom
    ...
    plot_fields_shimmed(np.abs(shimmed_fields), field_dict['XDim'],
                         ...
                         (0.0, 0.0, 0.0), mask_dict['sarmask_new'])
